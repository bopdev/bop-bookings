<?php 
/*
Plugin Name: Bop Bookings
Description: A simple bookings plugin
Version:     0.1.0
Author:      The Bop
...
//NOTE: REMEMBER TO REPLACE bop_bookings and bop-bookings in all files
 ...
 */
function bop_bookings_plugin_path( $path = '', $relative_file = false ){
 ...
 * @return void.
 */
function _bop_bookings_requirements_error(){
	global $wp_version_min_requirement, $php_version_min_requirement;
	?>
	<div class="notice notice-error">
		<p><?php printf( __( 'Error: This plugin requires WordPress v%s or higher (current: %s) and PHP v%s or higher (current: %s). You must be up to date or this plugin will only give this message and nothing more.', 'bop-bookings' ), $wp_version_min_requirement, $GLOBALS['wp_version'], $php_version_min_requirement, phpversion() ); ?></p>
	</div>
	<?php
}
...
if ( version_compare( $GLOBALS['wp_version'], $wp_version_min_requirement, '<' ) || version_compare( phpversion(), $php_version_min_requirement, '<' ) ) {
	...
	add_action( 'admin_notices', '_bop_bookings_requirements_error' );
	...
	require_once( bop_bookings_plugin_path( 'plugin-keeping.php' ) );
	
	/* Let's get started! */
	require_once( bop_bookings_plugin_path( 'plugin.php' ) );
